...
if [ -f /boot/firmware/config.txt ]; then
  ...
  # Enable I2C
  if [[ $ENABLE_I2C == "true" ]] && [ $(grep -ic "^dtparam=i2c_arm=on" /boot/firmware/config.txt) -eq 0 ]; then
  ...
  # Enable 1-Wire
  if [[ $ENABLE_1_WIRE == "true" ]] && [ $(grep -ic "^dtoverlay=w1-gpio" /boot/firmware/config.txt) -eq 0 ]; then
  ...
  # Enable camera
#   if [[ $ENABLE_CAMERA == "true" ]] && [ $(grep -ic "^gpu_mem=" /boot/firmware/config.txt) -eq 0 ]; then
#     echo "gpu_mem=128" >> /boot/firmware/config.txt
#     echo "Enabled Camera in boot config (1)"
#     REBOOT_REQUIRED=1
#   fi

#   if [[ $ENABLE_CAMERA == "true" ]] && [ $(grep -ic "^start_x=1" /boot/firmware/config.txt) -eq 0 ]; then
...
#   if [[ $ENABLE_CAMERA == "true" ]] && [ ! $(grep -ic "^dtoverlay=vc4-kms-v3d" /boot/firmware/config.txt) -eq 0 ]; then
#     # can't inline sed due to docker mount
#     cp /boot/firmware/config.txt /config.tmp
#     sed -i "/config.tmp" -e "s@^[ ]*dtoverlay=vc4-kms-v3d@#dtoverlay=vc4-kms-v3d@g"
#     cat /config.tmp > /boot/firmware/config.txt
#     rm /config.tmp
#     echo "Enabled Bullseye legacy Camera mode in boot config (1)"
#     REBOOT_REQUIRED=1
#   fi

#   if [[ $ENABLE_CAMERA == "true" ]] && [ ! $(grep -ic "^camera_auto_detect=.*" /boot/firmware/config.txt) -eq 0 ]; then
#     # can't inline sed due to docker mount
#     cp /boot/firmware/config.txt /config.tmp
...
#   if [[ $ENABLE_CAMERA == "true" ]] && [ $(grep -ic "^dtoverlay=vc4-fkms-v3d" /boot/firmware/config.txt) -eq 0 ] && [ $(grep -ic "^\[pi4\]" /boot/firmware/config.txt) -eq 1 ]; then
#     # can't inline sed due to docker mount
#     cp /boot/firmware/config.txt /config.tmp
...
#   fi
  ...
  # Enable serial
  if [[ $ENABLE_SERIAL == "true" ]] && [ $(grep -ic "^enable_uart=1" /boot/firmware/config.txt) -eq 0 ]; then
...
if [ -f /boot/firmware/cmdline.txt ]; then
  ...
  if [[ $ENABLE_CO2_SENSORS == "true" ]] && [ $(grep -ic "console=ttyAMA0,[0-9]\+ " /boot/firmware/cmdline.txt) -eq 1 ]; then
    # can't inline sed due to docker mount
    cp /boot/firmware/cmdline.txt /boot-cmdline.tmp
    sed -i "/boot-cmdline.tmp" -e "s@console=ttyAMA0,[0-9]\+ @@"
    cat /boot-cmdline.tmp > /boot/firmware/cmdline.txt
    rm /boot-cmdline.tmp
    echo "Enabled CO2 sensors in boot config (1)"
    REBOOT_REQUIRED=1
  fi

  if [[ $ENABLE_CO2_SENSORS == "true" ]] && [ $(grep -ic "console=serial0,[0-9]\+ " /boot/firmware/cmdline.txt) -eq 1 ]; then
    # can't inline sed due to docker mount
    cp /boot/firmware/cmdline.txt /boot-cmdline.tmp
