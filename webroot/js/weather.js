...
//			    .append(starssky)
  ...
  jQuery('.landscape .horizon .orb span').jqxTooltip({ trigger : 'click',
                              content: '<b>Loading...</b>',
                              showArrow: true,
                              showDelay: 5000,
                              position: 'bottom',
                                ...
                                content.append(jQuery('<span>').addClass('icon options edit').attr('title','Options (logged in)').bind('click',function (event){
                                                                                                                                    jQuery.ajax({
                                                                                                                                      ...
                                                                                                                                      dataType: 'json'
                                                                                                                                        ...
                                                                                                                                        var fields = [];
                                                                                                                                        fields.push({'name':'xmlsource','type':'text','value':result.value.xmlsource,'label':'XML Url','help':'Enter the full XML url from yo.nr'});
                                                                                                                                        showEditForm('Weather xml url',fields,'/weather/save','updateWeather()');
                                                                                                                                        //showEditForm(event.data.object.type() + ' sensor ' + event.data.object.name(),fields,'/weather/save','updateWeather()');
                                                                                                                                      }).fail(function() {
                                                                                                                                      });
                                                                                                                                    }));
                              }
...
function updateWeather(runonce) {
  ...
  jQuery.ajax({
    ...
    jQuery('.landscape .rise .date').text(moment(rise).format('HH:mm:ss')).attr({'title':(isDay ? 'Sun' : 'Moon') + ' rises at '+ moment(rise).format('dddd, MMMM D YYYY, HH:mm:ss') });
    jQuery('.landscape .set .date').text(moment(set).format('HH:mm:ss')).attr({'title':(isDay ? 'Sun' : 'Moon') + ' sets at '+ moment(set).format('dddd, MMMM D YYYY, HH:mm:ss') });
    ...
    jQuery('.landscape .horizon span').attr('title' , 'Local time: ' + moment().format('HH:mm:ss') + "\n" +
                                      ...
                                      .animate({'left' : leftPercentage + '%', 'top' : topPercentage + '%'},1000);
    ...
    if (sensorList[id] == undefined) {
      sensorList[id] = new TemperatureSensor( id,
                                              ...
                                              {min:result.value.temperature.minlimit, max: result.value.temperature.maxlimit},
                                              {min: result.value.temperature.min, max: result.value.temperature.max, current: result.value.temperature.current},
  ...
  if (!runonce) setTimeout(function() {updateWeather()}, weatherTimerTimeOut * 1000);
}
...
function Star() {
  ...
  this._canvas.css({'top':Math.floor((Math.random()*7)+1)+'%','left':Math.floor((Math.random()*90)+1) + '%'});
  ...
  this.animate = function() {
    if (this._canvas.is(':visible')) {
      this._canvas.animate({'background-size':'100%'},this._speed).animate({'background-size':'70%'},this._speed);
    }
  }

  this.draw();
  var me = this;
  setInterval(function(){ me.animate(); }, this._timeout);
}
