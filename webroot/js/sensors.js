...
function Sensor(id, name, limits, values, type, alarm, alarm_enabled, logging_enabled, indicator) {
  ...
  this._createcanvas = function() {
    ...
    this._canvas_options.bind('click', {
      object: this
    }, function(event) {
      var fields = [];
      fields.push({
        ...
        'label': 'ID',
        'help': 'Enter the md5 hash of the sensor address',
        'readonly': true
      });
      fields.push({
        'name': 'name',
        'type': 'text',
        'value': event.data.object.name(),
        'label': 'Name',
        'help': 'Enter a name of this sensor'
      });
      fields.push({
        'name': 'limit_min',
        'type': 'number',
        'value': event.data.object.limitMin(),
        'label': 'Minimum value',
      ...
      fields.push({
        'name': 'limit_max',
        'type': 'number',
        'value': event.data.object.limitMax(),
        'label': 'Maximum value',
        'help': 'Enter a number between -100 and 100'
      });
      if (event.data.object.type() == 'temperature') {
        ...
        fields.push({
          'name': 'indicator',
          'type': 'text',
          'value': event.data.object.indicator(),
          'label': 'Indicator',
          'help': 'Enter the indicator in degrees or farenheit'
        });
      }
      fields.push({
        'name': 'alarm_enabled',
        'type': 'switch',
        'value': event.data.object.alarm_enabled(),
        'label': 'Alarm enabled',
      ...
      fields.push({
        ...
        'type': 'switch',
        'value': event.data.object.logging_enabled(),
        'label': 'Logging enabled',
        'help': 'Enabled data logging'
      });
...
function HumiditySensor(id, name, limits, values, alarm, alarm_enabled, logging_enabled, indicator) {
  ...
  this.animate = function() {
    ...
    this._canvas_animation.animate({
      'opacity': Math.round(this.val()) / 100
    }, 2500);
  };
  this.start();
}
...
function TemperatureSensor(id, name, limits, values, alarm, alarm_enabled, logging_enabled, indicator) {
  ...
  this.animate = function() {
    ...
    jQuery(this._canvas_animation).find('span.range').css({
      'top': (95 - this.limitMax()) + '%',
      'height': Math.abs(this.limitMax() - this.limitMin()) + '%'
    });
    jQuery(this._canvas_animation).find('span.minmax').css({
      'top': (95 - this.valueMax()) + '%',
      'height': Math.abs(this.valueMax() - this.valueMin()) + '%'
    });
    // Animate the temperature
    jQuery(this._canvas_animation).find('span.liquid').animate({
      'top': (95 - this.val()) + '%'
    }, 2500);
  }
  this.start();
