...
function loadSystemDashboard() {
  ...
  table.append(jQuery('<tr>') .append(jQuery('<td>').append(jQuery('<span>').addClass('icon about pointer').attr('title','About')))
                              .append(jQuery('<td>').attr({'colspan':2}).append(jQuery('<h1>').text('System')))
                              .append(jQuery('<td>').append(jQuery('<span>').addClass('updater')))
  );
  // Last update row
  table.append(jQuery('<tr>') .append(jQuery('<td>').attr({'colspan':4,'id':'system_update'}).text('updating...')));
  table.append(jQuery('<tr>') .append(jQuery('<td>').attr({'colspan':4,'id':'system_uptime'}).text('updating...')));
  ...
  table.append(jQuery('<tr>') .append(jQuery('<td>').append(jQuery('<div>').attr({'id':'gauge_memory'}).css({'width':'5.5em','height':'5.5em'})))
                              .append(jQuery('<td>').append(jQuery('<div>').attr({'id':'gauge_cpuspeed'}).css({'width':'5.5em','height':'5.5em'})))
                              .append(jQuery('<td>').append(jQuery('<div>').attr({'id':'gauge_cpuload'}).css({'width':'5.5em','height':'5.5em'})))
                              .append(jQuery('<td>').append(jQuery('<div>').attr({'id':'gauge_cputemp'}).css({'width':'5.5em','height':'5.5em'})))
  );

  table.append(jQuery('<tr>') .append(jQuery('<td>').append(jQuery('<div>').attr({'id':'gauge_wattage'}).css({'width':'5.5em','height':'5.5em'})))
                              .append(jQuery('<td>').append(jQuery('<div>').attr({'id':'gauge_waterflow'}).css({'width':'5.5em','height':'5.5em'})))
  );
...
function updateSystem() {
  jQuery.ajax({
    ...
    //jQuery('#system .updater').css({opacity: 0.0, visibility: "visible"}).animate({opacity: 1.0},500).animate({opacity: 0},500);
  ...
  setTimeout(function() {updateSystem()}, systemTimerTimeOut * 1000);
}
