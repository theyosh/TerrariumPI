...
function loadEnvironmentDashboard() {
  ...
  table.append(jQuery('<tr>').append(jQuery('<td>').append(jQuery('<span>').addClass('icon authentication edit loggedout').attr('title', 'Login to make changes')))
    ...
    .append(jQuery('<td>').append(jQuery('<span>').addClass('icon counter').attr({
      'id': 'env_counter'
    })))
  );
  ...
  table.append(jQuery('<tr>').attr({
    ...
    .append(jQuery('<td>').text('enabled').append(jQuery('<span>').addClass('icon inactive enabled'))
      .append('trigger').append(jQuery('<span>').addClass('icon inactive trigger').attr('title', 'Trigger inactive'))
      .append('alarm').append(jQuery('<span>').addClass('icon inactive alarm').css({
        'display': 'none'
      }).attr('title', 'Alarm!')))
    .append(jQuery('<td>').append(jQuery('<div>').addClass('animation inactive')
  ...
  table.append(jQuery('<tr>').attr({
    ...
    .append(jQuery('<td>').text('enabled').append(jQuery('<span>').addClass('icon inactive enabled'))
      .append('trigger').append(jQuery('<span>').addClass('icon inactive trigger').attr('title', 'Trigger inactive'))
      .append('alarm')
      .append(jQuery('<span>').addClass('icon inactive alarm').css({
        'display': 'none'
      }).attr('title', 'Alarm!')))
    .append(jQuery('<td>').append(jQuery('<div>').addClass('animation inactive'))
      .append(jQuery('<div>').addClass('period')))
...
function showEnvironmentLightsSettingsForm() {
  jQuery.ajax({
    ...
    fields.push({
      'name': 'enabed',
      'type': 'switch',
      'value': result.value.enabled,
      'label': 'Lights enabled',
      'help': 'Enabled alarm notifications'
    });
    ...
    fields.push({
      ...
      'value': 'Weather',
      'label': 'Light modus',
      'checked': result.value.trigger_on == 'sunrise' || result.value.trigger_off == 'sunset'
    });
    fields.push({
      'name': 'modus',
      'type': 'radio',
    ...
    fields.push({
      'name': 'on',
      'type': 'text',
      'value': result.value.trigger_on,
      'label': 'Lights on',
      'help': 'Enter the time when the lights should be on'
    });
    fields.push({
      'name': 'off',
      'type': 'text',
      'value': result.value.trigger_off,
      'label': 'Lights off',
      'help': 'Enter the time when the lights should be off'
    });
    ...
    fields.push({
      ...
      'type': 'list',
      'value': '',
      'label': 'Select the power switches',
      'help': 'Select the right power switches'
    });
    ...
    fields.push({
      'name': 'maxhours',
      'type': 'number',
      'value': Math.round(result.value.max_duration / 3600),
      'label': 'Maximum hours of light',
      'help': 'Enter a number between -100 and 100'
    });
    fields.push({
      'name': 'minhours',
      'type': 'number',
      'value': Math.round(result.value.min_duration / 3600),
      'label': 'Minimum hours of light',
      'help': 'Enter a number between -100 and 100'
    });
    fields.push({
      'name': 'shifthours',
      'type': 'number',
      'value': Math.round(result.value.time_shift / 3600),
      'label': 'Shift light hours',
      'help': 'Enter a number between -100 and 100'
    });
    ...
    jQuery('#radio_modus').jqxButtonGroup('setSelection', jQuery('div#' + modus).index());
  });
}
...
function showEnvironmentHeaterSettingsForm() {
  jQuery.ajax({
    ...
    fields.push({
      ...
      'type': 'list',
      'value': '',
      'label': 'Select the temperature sensors',
      'help': 'Select the right sensors'
    });
    fields.push({
      'name': 'dayactive',
      'type': 'switch',
      'value': result.value.day_active,
      'label': 'Heater enabled during the day',
      'help': 'Enabled alarm notifications'
    });
...
function showEnvironmentHumiditySettingsForm() {
  jQuery.ajax({
    ...
    fields.push({
      ...
      'type': 'list',
      'value': '',
      'label': 'Select the humidity sensors',
      'help': 'Select the right sensors'
    });
    fields.push({
      'name': 'timeout',
      'type': 'text',
      'value': result.value.switch_timeout,
      'label': 'Humidity wait timeout (seconds)',
      'help': 'Enter the time when the lights should be on'
    });
    fields.push({
      'name': 'duration',
      ...
      'label': 'Humidity spray duration (seconds)',
      'help': 'Enter a number between -100 and 100'
    });
    fields.push({
      'name': 'night_enabled',
      'type': 'switch',
      'value': result.value.night_active,
      'label': 'Humidity enabled when the lights are off',
      'help': 'Enabled alarm notifications'
    });
...
function loadEnvironmentHumidity(data) {
  ...
  jQuery('#environment #env_humidity .animation').jqxGauge({
    ranges: ranges,
    caption: {
      value: Math.round(data.avg_val * 100) / 100 + '%'
    }
  });
