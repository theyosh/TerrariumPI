...
function Switch(id, name, nr, state, type, wattage, waterflow) {
  ...
  this._createcanvas = function() {
    this._canvas = jQuery('<div>').attr({
      'id': 'switch_' + this._type + '_' + this._id
    }).addClass(this._type).addClass(this.state() ? 'on' : 'off');
    ...
    this._canvas_options.bind('click', {
      ...
      fields.push({
        ...
        'label': 'ID',
        'help': 'Enter the md5 hash of the switch address',
        'readonly': true
      });
      fields.push({
        'name': 'type',
        'type': 'text',
        'value': event.data.object.type(),
        'label': 'Type',
        'help': 'Enter switch type',
        'readonly': true
      });
      fields.push({
        'name': 'name',
        ...
        'label': 'Name',
        'help': 'Enter a name of this switch'
      });
      fields.push({
      ...
      fields.push({
        'name': 'wattage',
        'type': 'number',
        'value': event.data.object.wattage(),
        'label': 'Power usage in Watt',
        'help': 'Enter the amount of power this switch consumes when switched on'
      })
      fields.push({
        'name': 'waterflow',
        'type': 'number',
    ...
    this._canvas_switch.bind('click', {
      object: this
    }, function(event) {
      if (loggedin) event.data.object.toggle();
    });

    this._canvas_lightbulb.jqxTooltip({
      trigger: 'click',
      content: '<b>Loading...</b>',
      showArrow: true,
      showDelay: 5000,
