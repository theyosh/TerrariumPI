...
if (isset($_GET['sortdatafield']))
	...
	if (($sortfield != NULL) && (in_array($sortfield, $sortfields)))
		...
		if ($sortorder == "desc")
			{
			$query = "SELECT SQL_CALC_FOUND_ROWS CompanyName, ContactName, ContactTitle, Address, City, Country FROM customers ORDER BY" . " " . $sortfield . " DESC LIMIT ?,?";
			}
		  else if ($sortorder == "asc")
			{
			$query = "SELECT SQL_CALC_FOUND_ROWS CompanyName, ContactName, ContactTitle, Address, City, Country FROM customers ORDER BY" . " " . $sortfield . " ASC LIMIT ?,?";
			}
		}
	}
