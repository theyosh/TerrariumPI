...
#   # 棕 = Brown  = VCC
  ...
  def _load_hardware(self):
    ...
    device  = SHT20(1 if len(address) == 1 or int(address[1]) < 1 else int(address[1]), resolution=SHT20.TEMP_RES_14bit)
    return device

  def _get_data(self):
    data = None
    sensor = self.device.read_all()
    data = {}
    data['temperature'] = sensor[0]
    data['humidity']    = sensor[1]
    return data
