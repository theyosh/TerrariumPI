...
![Homer Simpsons: DOH!](/assets/img/doh.webp){: .right .invert width="150" }IApparently there was a bug in the code for many, many years :(. The settle time value was calculated from the time the relay was toggled on. But when the relay on duration is higher than the settle time value, the settle time was useless.
