...
function showTimerGraph() {
  ...
  const begin_time = Date.parse(
    today + jQuery("input#begin_time").val() + ":00"
  );
  ...
  if (begin_time && end_time) {
    ...
    const dataSetTotal = {
      ...
      order: 3,
      data: [
        { x: new Date(begin_time - 0.5 * 3600000), y: 0 },
        { x: new Date(begin_time), y: 3 },
        { x: new Date(end_time), y: 0 },
        { x: new Date(end_time + 0.5 * 3600000), y: 0 }
      ]
    };
    ...
    while (new Date(period_start + on_duration) <= new Date(end_time)) {
      ...
      let run_duration =
        running_time == "" || running_time == 0 || settle_duration == 0
          ? (period_end - period_start) / 1000
          : running_time;
    ...
    if (timerGraph === null) {
      timerGraph = new Chart(document.getElementById("timeGraph"), config);
    } else {
