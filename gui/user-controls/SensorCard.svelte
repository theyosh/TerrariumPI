...
{#if sensor}
  <Card loading={false} noPadding={true} {removeParent} class="{sensor.id} {$$props.class || ''}">
    <svelte:fragment slot="header">
      ...
      {#if $sensors[sensor.id]}
        ...
        <small
          class="text-muted"
          class:ml-2={!$sensors[sensor.id].excluded && !$sensors[sensor.id].error && !$sensors[sensor.id].alarm}
    ...
    <svelte:fragment slot="tools">
      ...
      <CardSettingsTools>
        <button
          class="dropdown-item"
          title={$_('sensors.actions.export', { default: 'Export sensor' })}
          on:click={exportAction}
        >
          <i class="fas mr-2" class:fa-file-export={!exporting} class:fa-spinner={exporting} class:fa-spin={exporting}
          ></i>{$_('sensors.actions.export', { default: 'Export sensor' })}
        </button>
    ...
    <div class="row">
      ...
      >
        ...
        {#if $sensors[sensor.id]?.calibration?.light_on_off_threshold > 0}
          <i
            class="fa-regular fa-lightbulb light_on_off_threshold"
            class:on={$sensors[sensor.id].value > $sensors[sensor.id].calibration.light_on_off_threshold}
