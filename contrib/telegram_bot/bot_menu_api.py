...
def handle(msg):  # Wait message from user in telegram
    ...
    # Simple calendar on month
        ...
        calendar_print = (
            calendar.LocaleTextCalendar(firstweekday=0, locale="ru_RU.UTF-8")
            .formatmonth(current_year, current_month, 0, 0)
            .encode("utf8")
        )
    ...
    # Info about raspberry pi
        ...
        for_print = str(
            ...
            + format(api_req_get_status_rpi["temperature"], ".2f")
            + " C"
            + "\nFree memory is "
            + format(api_req_get_status_rpi["memory"]["free"] / 1024 / 1024, ".1f")
            + " MB"
            + " of total "
            + format(api_req_get_status_rpi["memory"]["total"] / 1024 / 1024, ".1f")
            + " MB"
        )
    ...
    # Menu - "Photo"
        ...
        camera.annotate_text = str(datetime.datetime.now().strftime("%d %B %Y %H:%M"))
    ...
    # Menu - "Management"
        ...
        bot.sendMessage(
            ...
            reply_markup=ReplyKeyboardMarkup(
                keyboard=[
                    [
                        KeyboardButton(text=str("Toggle " + dict_switches_name.get("Switch1"))),
                        KeyboardButton(text=str("Toggle " + dict_switches_name.get("Switch2"))),
                    ],
                    [
                        KeyboardButton(text=str("Toggle " + dict_switches_name.get("Switch3"))),
                        KeyboardButton(text=str("Toggle " + dict_switches_name.get("Switch4"))),
                    ],
    ...
    # Toggle switches
        ...
        requests.post(
            url=str("http://localhost:8090/api/switch/toggle/" + dict_switches_id.get("Switch1")),
            auth=(login_user_Terrarium_Pi, login_password_Terrarium_Pi),
        )
        for_print = requests.get(url='http://localhost:8090/api/switches/dict_switches_id.get("Switch1")').json()[
            "switches"
        ][0]["state"]
        print(for_print)
        bot.sendMessage(chat_id, for_print)
        ...
        requests.post(
            url=str("http://localhost:8090/api/switch/toggle/" + dict_switches_id.get("Switch2")),
            auth=(login_user_Terrarium_Pi, login_password_Terrarium_Pi),
        )
        for_print = requests.get(url='http://localhost:8090/api/switches/dict_switches_id.get("Switch2")').json()[
            "switches"
        ][0]["state"]
        print(for_print)
        bot.sendMessage(chat_id, for_print)
        ...
        requests.post(
            url=str("http://localhost:8090/api/switch/toggle/" + dict_switches_id.get("Switch3")),
            auth=(login_user_Terrarium_Pi, login_password_Terrarium_Pi),
        )
        for_print = requests.get(url='http://localhost:8090/api/switches/dict_switches_id.get("Switch3")').json()[
            "switches"
        ][0]["state"]
        print(for_print)
        bot.sendMessage(chat_id, for_print)
        ...
        requests.post(
            url=str("http://localhost:8090/api/switch/toggle/" + dict_switches_id.get("Switch4")),
            auth=(login_user_Terrarium_Pi, login_password_Terrarium_Pi),
        )
        for_print = requests.get(url='http://localhost:8090/api/switches/dict_switches_id.get("Switch4")').json()[
            "switches"
        ][0]["state"]
        print(for_print)
        bot.sendMessage(chat_id, for_print)
