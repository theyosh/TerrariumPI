...
def read_sensor_settings(config_file):
  ...
  for section in settings.sections():
    ...
    relays.append({
      ...
      'replacement' : None if 'last_replacement_date' not in settings[section] or '2019-01-01' == settings[section]['last_replacement_date'] else settings[section]['last_replacement_date']
    })
...
def convert_data(relay_data, old_db, new_db):
  ...
  for relay in relay_data:
    ...
    if relay['replacement'] is not None:
      ...
      new_cur.execute('UPDATE Relay SET replacement = :replacement WHERE id = :id', {'replacement' : relay['replacement'], 'id' : relay["new_id"]})
      new_db.commit()

    # Copy the relay data records from old db into new db.
    new_cur.execute(f'''INSERT OR IGNORE INTO RelayHistory (relay, timestamp, value, wattage, flow)
...
if __name__ == "__main__":
  ...
  print(f'Found {len(new_relays)} out of {len(old_relays)} are found. Below is a summary of the founded relays that can converted.\n')
  for relay in new_relays:
    print(f'Relay \'{relay["name"]}\' of type \'{relay["type"]}\' at address \'{relay["address"]}\'. Old ID: {relay["old_id"]} => New ID: {relay["new_id"]}')
