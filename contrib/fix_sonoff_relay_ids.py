...
if available_disk_space < 2.5 * DATABASE.stat().st_size:
  exit(f'Not enough free space on disk. Found {sizeof_fmt(available_disk_space)}, needs at least {sizeof_fmt(2.5 * DATABASE.stat().st_size)}')
            ...
            if old_id in setup[area_part]['relays']:
              ...
              for relay_tweak in relay_tweaks:
                if setup[area_part].get(f'relay_{relay_tweak}_{old_id}',None):
                  setup[area_part][f'relay_{relay_tweak}_{new_id}'] = setup[area_part][f'relay_{relay_tweak}_{old_id}']
