    ...
    try:
        ...
        for line_str in Pygtail(tplog_str):
            ...
            for lvlsearch_str in lvlsearch_list:
                if debug_bool:
                    logfile_obj.write("%s - searching Lvl-String: %s\n" % (datetime.datetime.now(), lvlsearch_str))
                for modulesearch_str in modulesearch_list:
                    ...
                    if lvlsearch_str in line_str and modulesearch_str in line_str:
                        if (
                            not "999" in line_str
                        ):  # This is to prevent getting a message, everytime a sensor reports a false value
                            ...
                            if not debug_bool:
                                if not ntfyconfig_str:
                                    ...
                                    ntfy.notify(
                                        message_str,
                                        module_str.strip() + "-" + lvl_str.strip(),
                                        config=load_config(ntfyconfig_str.strip()),
                                    )
                            else:
                                logfile_obj.write(
